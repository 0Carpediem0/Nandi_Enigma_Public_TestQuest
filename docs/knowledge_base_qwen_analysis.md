...
3. **Опционально позже:**
   - ~~Подключить конфигурацию полнотекстового поиска для русского (`russian`), чтобы улучшить поиск по разным формам слов.~~ **Сделано:** в `init_database.py` и в `repositories.search_knowledge_base` используется конфиг `russian`; для уже существующей БД нужно один раз выполнить `python init_database.py` (без `--drop-existing`), чтобы колонка `search_vector` пересоздалась с `russian`.
   - ~~Если понадобится семантика — вернуть колонку `embedding` (pgvector) и заполнять её эмбеддингами.~~ **Сделано:** колонка `embedding vector(384)`, индекс ivfflat; сервис `embedding_service.get_embedding()` (HF feature-extraction); `POST /kb/refresh-embeddings` заполняет эмбеддинги; поиск с `use_vector=true` и в `/kb/ask` (параметр `use_vector`). В .env задать `EMBEDDING_MODEL` (по умолчанию `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`) и `HF_TOKEN`.
